...
@section('content')
    <div class="dashboard-wrapper">
        ...
        <!-- 1. Header Section -->
            <div class="row align-items-center mb-5 position-relative">
                <div class="col-md-6">
                    ...
                    <h2 class="fw-black text-heading mb-1 display-6">لوحة التحكم <span class="text-primary">الذكية</span></h2>
                ...
                <div class="col-md-6 text-md-end mt-4 mt-md-0">
                    <div
                        class="glass-card d-inline-block px-4 py-3 rounded-4 border border-secondary border-opacity-10 border-opacity-10 shadow-lg">
                        <div class="d-flex align-items-center gap-4">
                            <div class="text-end">
                                <div id="dashboard-clock" class="fw-black text-heading fs-3 lh-1 tracking-tight">--:--:--</div>
                                <div class="text-primary small fw-bold mt-1">
                                    {{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}
                                </div>
                            </div>
                            <div class="vr bg-white opacity-10" style="height: 40px;"></div>
                            <div class="pulse-icon bg-primary bg-opacity-30 text-heading rounded-circle d-flex align-items-center justify-content-center shadow-sm"
            ...
            <div class="row g-4 mb-5">
                <div class="col-md-4">
                    <div
                        ...
                        <div class="d-flex align-items-baseline gap-2">
                            <h2 class="text-heading fw-black mb-0 display-5">
                ...
                <div class="col-md-4">
                    <div
                        ...
                        <h2 class="text-heading fw-black mb-0 display-5">{{ number_format($stats['active_employees']) }}
                        </h2>
                    </div>
                </div>
                <div class="col-md-4">
                    <div
                        class="stat-card glass-card p-4 border-start border-info border-4 rounded-4 shadow-lg overflow-hidden position-relative bg-gradient-info h-100">
                        <div class="row align-items-center h-100">
                            <div class="col-md-8">
                                <div class="badge bg-info bg-opacity-10 text-info mb-3 text-white">السيولة والمرتبات
                                </div>
                                <h6 class="text-secondary fw-bold mb-1 text-heading opacity-75">ميزانية الرواتب</h6>
                                <h2 class="text-heading fw-black mb-0 display-5">
                                    {{ number_format($stats['total_salary'], 0) }} <small
                                        class="fs-6 fw-normal opacity-50">ج.م</small>
                                </h2>
                            </div>
                            <div class="col-md-4 text-md-end">
                                <div
                                    class="icon-circle bg-white bg-opacity-10 text-body p-3 d-inline-block rounded-circle">
            ...
            <!-- 3. Main Split Layout -->
            <div class="row g-4 mb-5">
                <!-- Left Column: System Hub + Alerts -->
                <div class="col-lg-8">
                    <!-- System Hub (Moved Here) -->
                    <h5 class="text-heading fw-black mb-4 px-2">مركز إدارة العمليات System Hub</h5>
                    <div class="row g-4 mb-5">
                        @php
                            $modules = [
                                [
                        ...
                        @foreach($modules as $mod)
                            <div class="col-md-6">
                                <a href="{{ route($mod['route']) }}"
                                    class="card-hub glass-card h-100 p-4 shadow-lg rounded-4 text-decoration-none border border-secondary border-opacity-10 border-opacity-5 d-block position-relative overflow-hidden">
                                    <div class="d-flex align-items-center gap-3">
                                        <div class="icon-box-hub bg-{{ $mod['color'] }} bg-opacity-10 text-{{ $mod['color'] }} mb-0" style="width: 50px; height: 50px;">
                                            <i class="bi {{ $mod['icon'] }} fs-3"></i>
                                        </div>
                                        <div>
                                            <h6 class="text-heading fw-black mb-1">{{ $mod['name'] }}</h6>
                                    ...
                                    <div class="col-md-6">
                                        <div class="alert-box border border-danger border-opacity-25 rounded-3 p-3 h-100">
                                            ...
                                            @foreach($expiringDocuments->take(3) as $doc)
                                                <div
                                                    class="d-flex justify-content-between align-items-center py-2 border-bottom border-secondary border-opacity-10 border-opacity-10 last-border-0">
                                                    <span class="text-heading small fw-bold">{{ $doc->employee->full_name }}</span>
                                    ...
                                    <div class="col-md-6">
                                        <div class="alert-box border border-warning border-opacity-25 rounded-3 p-3 h-100">
                                            ...
                                            @foreach($pendingLeaves->take(3) as $leave)
                                                <div
                                                    class="d-flex justify-content-between align-items-center py-2 border-bottom border-secondary border-opacity-10 border-opacity-10 last-border-0">
                                                    <span class="text-heading small fw-bold">{{ $leave->employee->full_name }}</span>
                ...
                <!-- Right Column: Attendance Widget (Moved to Side) -->
                <div class="col-lg-4">
                    <div
                        class="glass-card card-hover-plus border-0 shadow-lg h-100 overflow-hidden rounded-4 position-relative">
                        <div class="card-body p-5 text-center position-relative d-flex flex-column justify-content-center h-100">
                            <div class="mb-4">
                                <h5 class="text-heading fw-bold mb-1">تسجيل الوقت</h5>
                            ...
                            @if($attendanceStatus == 'not_linked')
                                <div class="py-4">
                                    ...
                                    <h6 class="text-heading fw-bold">حساب غير مرتبط</h6>
                            ...
                            @elseif($attendanceStatus == 'not_checked_in')
                                <div class="py-4">
                                    <form action="{{ route('hr.attendance.check-in') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="attendance-btn btn-check-in mx-auto">
                            ...
                            @elseif($attendanceStatus == 'checked_in')
                                <div class="py-4">
                                    ...
                                    <form action="{{ route('hr.attendance.check-out') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="attendance-btn btn-check-out mx-auto">
                                            <div class="btn-content">
                            ...
                            @elseif($attendanceStatus == 'checked_out')
                                <div class="py-5">
                                    ...
                                    <h5 class="text-heading fw-bold">تم إنهاء العمل بنجاح</h5>
        ...
        <style>
            /* :root overrides removed to respect global theme */
            .dashboard-wrapper {
                position: relative;
                padding: 20px 0;
            }
