            ...
            <div class="row g-4 m-0">
                ...
                <div class="col-md-4">
                    <div class="p-3 rounded-3 border h-100"
                        ...
                        <h5 class="mb-3 text-white">{{ $shift->user->name }}</h5>

                        <p class="text-secondary small mb-1">توقيت الفتح</p>
                        <p class="fw-bold mb-3 text-white">{{ $shift->opened_at->format('Y-m-d H:i') }}</p>

                        @if($shift->closed_at)
                            <p class="text-secondary small mb-1">توقيت الإغلاق</p>
                            <p class="fw-bold text-white">{{ $shift->closed_at->format('Y-m-d H:i') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="col-md-8">
                    <div class="table-responsive border rounded-3 overflow-hidden">
                        <table class="table table-hover mb-0 align-middle">
                            ...
                            <tbody>
                                <tr>
                                    <td class="ps-3 bg-transparent text-white">النقدية الافتتاحية (العهدة)</td>
                                    <td class="text-end pe-3 fw-bold text-white">
                                        {{ number_format($stats['opening_cash'], 2) }} EGP
                                    </td>
                                </tr>
                                <tr>
                                    <td class="ps-3 bg-transparent text-white">إجمالي المبيعات (نقداً)</td>
                                    <td class="text-end pe-3 text-success fw-bold">+
                                        {{ number_format($stats['total_cash'], 2) }} EGP
                                    </td>
                                </tr>
                                <tr>
                                    <td class="ps-3 bg-transparent text-white">إجمالي المصروفات</td>
                                    <td class="text-end pe-3 text-danger fw-bold">-
                                        {{ number_format($stats['total_expenses'], 2) }} EGP
                                    </td>
                                </tr>
                                <tr>
                                    <td class="ps-3 bg-transparent text-white">إجمالي المرتجعات (نقداً)</td>
                                    <td class="text-end pe-3 text-danger fw-bold">-
                                ...
                                <tr class="table-primary border-top border-primary border-2"
                                    ...
                                    <td class="ps-3 fw-bold text-white" style="background: transparent;">النقدية المتوقعة في
                                        الدرج</td>
                                ...
                                <tr class="table-secondary" style="background-color: rgba(108, 117, 125, 0.2) !important;">
                                    <td class="ps-3 text-white" style="background: transparent;">النقدية الفعلية (المسلمة)
                                    </td>
                                    <td class="text-end pe-3 fw-bold text-white" style="background: transparent;">
                                ...
                                <tr class="{{ $stats['difference'] < 0 ? 'table-danger' : ($stats['difference'] > 0 ? 'table-warning' : 'table-success') }}"
                                    ...
                                    <td class="ps-3 fw-bold text-white" style="background: transparent;">الفارق / العجز</td>
                    ...
                    <div class="row row-cols-1 row-cols-md-3 g-3">
                        ...
                        <div class="col">
                            <div class="p-3 border rounded shadow-sm text-center"
                                ...
                                <h4 class="mb-0 text-white">{{ number_format($stats['total_sales'], 2) }}</h4>
                            </div>
                        </div>
                    </div>
                </div>
