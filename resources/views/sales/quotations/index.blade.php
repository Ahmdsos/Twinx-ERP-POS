    ...
    <div class="container-fluid p-0">
        ...
        <div class="d-flex justify-content-between align-items-end mb-5">
            <div>
                <h2 class="fw-bold text-heading mb-2 tracking-wide display-6">
                    <i class="bi bi-file-earmark-text text-info me-2"></i>{{ __('Quotations') }}</h2>
        ...
        <div class="row g-4 mb-5">
            ...
            <div class="col-md-4">
                <div class="glass-card stat-card h-100 position-relative border-0 shadow-lg overflow-hidden">
                    <div class="position-absolute top-0 end-0 p-4 opacity-10">
                        <i class="bi bi-hourglass-split display-1 text-body"></i>
                    </div>
                    <div class="p-4 position-relative z-1">
                        ...
                        <h1 class="text-heading fw-bold mb-1 display-5">{{ number_format($stats['pending'] ?? 0) }}</h1>
            ...
            <div class="col-md-4">
                <div class="glass-card stat-card h-100 position-relative border-0 shadow-lg overflow-hidden">
                    <div class="position-absolute top-0 end-0 p-4 opacity-10">
                        <i class="bi bi-cash-coin display-1 text-body"></i>
                    </div>
                    <div class="p-4 position-relative z-1">
                        <div
                            class="badge bg-surface/10 text-body mb-3 backdrop-blur-md border border-secondary border-opacity-10/10 px-3 py-1 rounded-pill">
                        ...
                        <h1 class="text-heading fw-bold mb-1 display-5">{{ number_format($stats['total_value'] ?? 0, 2) }}
                        </h1>
            ...
            <div class="col-md-4">
                <div class="glass-card stat-card h-100 position-relative border-0 shadow-lg overflow-hidden">
                    <div class="position-absolute top-0 end-0 p-4 opacity-10">
                        <i class="bi bi-check-circle display-1 text-body"></i>
                    </div>
                    <div class="p-4 position-relative z-1">
                        <div
                            class="badge bg-surface/10 text-body mb-3 backdrop-blur-md border border-secondary border-opacity-10/10 px-3 py-1 rounded-pill">
                        ...
                        <h1 class="text-heading fw-bold mb-1 display-5">{{ number_format($stats['accepted'] ?? 0) }}</h1>
        ...
        <div class="glass-panel p-4 mb-4 rounded-4 border border-secondary border-opacity-10/10 shadow-lg position-relative overflow-hidden">
            ...
            <form action="{{ route('quotations.index') }}" method="GET" class="position-relative z-1">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        <label class="text-gray-300 small fw-bold mb-2 ps-1">{{ __('Customer') }}</label>
                    ...
                    <div class="col-md-2">
                        <label class="text-gray-300 small fw-bold mb-2 ps-1">{{ __('Status') }}</label>
                        <select name="status" class="form-select glass-select text-body shadow-none">
                    ...
                    <div class="col-md-4 d-flex gap-2">
                        <button type="submit" class="btn btn-gradient-info w-100 fw-bold shadow-neon-blue py-2">
                            <i class="bi bi-funnel-fill me-2"></i>{{ __('Filter') }}</button>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="border-collapse: separate; border-spacing: 0;">
                    <thead class="bg-surface/5">
                        <tr>
                            <th class="py-3 ps-4 text-gray-400 fw-normal">{{ __('Quotation Number') }}</th>
                            ...
                            <th class="py-3 text-gray-400 fw-normal text-end">{{ __('Total') }}</th>
                            ...
                            <tr class="hover-bg-surface-5">
                                <td class="ps-4">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span
                                            class="font-monospace fw-bold text-body">{{ $quotation->quotation_number }}</span>
                                    </div>
                                </td>
                                <td>
                                    @if($quotation->customer)
                                        <span class="fw-bold text-body">{{ $quotation->customer->name }}</span>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="text-body small">{{ $quotation->quotation_date->format('Y-m-d') }}</span>
                                        <small class="text-gray-500 x-small">ينتهي:
                                            {{ $quotation->valid_until ? $quotation->valid_until->format('Y-m-d') : '-' }}</small>
                                    </div>
                                </td>
                                <td>
                                    <span class="badge {{ $quotation->status->badgeClass() }} border border-secondary border-opacity-10/10">
                                        {{ $quotation->status->label() }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    <h5 class="mb-0 fw-bold text-heading text-glow">{{ number_format($quotation->total, 2) }}</h5>
                                </td>
                                <td class="pe-4 text-end">
                                    <div class="btn-group">
                                        <a href="{{ route('quotations.show', $quotation->id) }}"
                                            class="btn btn-sm btn-icon-glass text-info" title="{{ __('View') }}">
                                            <i class="bi bi-eye-fill"></i>
                                        </a>
                                        <a href="{{ route('quotations.edit', $quotation->id) }}"
                                            class="btn btn-sm btn-icon-glass text-warning" title="{{ __('Edit') }}">
                                            <i class="bi bi-pencil-fill"></i>
                                        </a>
                                        <a href="{{ route('quotations.print', $quotation->id) }}" target="_blank"
                                            class="btn btn-sm btn-icon-glass text-secondary" title="{{ __('Print') }}">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center opacity-50">
                                        ...
                                        <h5 class="text-gray-400">{{ __('No quotations found') }}</h5>
                                        <p class="text-gray-600 small">اضغط على زر "{{ __('New Quotation') }}" للبدء</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
