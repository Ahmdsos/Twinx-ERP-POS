...
class POSService
{
    ...
    public function checkout(array $data): SalesInvoice
    {
        return DB::transaction(function () use ($data) {
            ...
            // CRITICAL: Refresh invoice to get DB-calculated totals from SalesInvoiceLine::boot()
            $invoice->refresh();
    ...
    protected function createInvoiceJournalEntry(SalesInvoice $invoice, array $payments): void
    {
        ...
        // Guard: Essential accounts must exist
        if (!$cashAccount) {
            throw new \RuntimeException("حساب النقدية (كود: {$cashCode}) غير موجود. يرجى التأكد من إعداد دليل الحسابات أولاً.");
        }
        if (!$salesAccount) {
            throw new \RuntimeException("حساب إيرادات المبيعات (كود: {$salesCode}) غير موجود. يرجى التأكد من إعداد دليل الحسابات أولاً.");
        }
