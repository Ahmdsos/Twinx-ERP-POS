    ...
    <div class="dashboard-wrapper">
        ...
        <div class="row g-4 mb-4">
            ...
            <div class="col-md-3">
                <div class="glass-card rounded-4 p-4 border-white border-opacity-10">
                    <div class="d-flex align-items-center gap-3">
                        <div class="icon-box rounded-circle bg-success bg-opacity-10 text-success d-flex align-items-center justify-content-center"
                            style="width: 48px; height: 48px;">
            ...
            <div class="col-md-3">
                <div class="glass-card rounded-4 p-4 border-white border-opacity-10">
                    <div class="d-flex align-items-center gap-3">
                        <div class="icon-box rounded-circle bg-warning bg-opacity-10 text-warning d-flex align-items-center justify-content-center"
                            style="width: 48px; height: 48px;">
            ...
            <div class="col-md-3">
                <div class="glass-card rounded-4 p-4 border-white border-opacity-10">
                    <div class="d-flex align-items-center gap-3">
                        <div class="icon-box rounded-circle bg-secondary bg-opacity-10 text-secondary d-flex align-items-center justify-content-center"
                            style="width: 48px; height: 48px;">
            ...
            <form action="{{ route('hr.delivery.index') }}" method="GET" class="row g-3 align-items-end">
                <div class="col-md-5">
                    ...
                    <div class="input-group">
                        <span
                            class="input-group-text bg-white bg-opacity-5 border-white border-opacity-10 text-secondary"><i
                ...
                <div class="col-md-3">
                    ...
                    <select name="status"
                        ...
                        <option value="available" class="bg-dark" {{ request('status') == 'available' ? 'selected' : '' }}>
                            متاح</option>
                        <option value="on_delivery" class="bg-dark" {{ request('status') == 'on_delivery' ? 'selected' : '' }}>في مهمة</option>
                        <option value="offline" class="bg-dark" {{ request('status') == 'offline' ? 'selected' : '' }}>غير
                            متصل</option>
                        <option value="suspended" class="bg-dark" {{ request('status') == 'suspended' ? 'selected' : '' }}>
                            موقوف</option>
                    </select>
                </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead>
                        <tr class="bg-white bg-opacity-5">
                            ...
                            <th class="py-3 text-secondary small fw-bold border-0 text-center">الوضعية الحالية</th>
                            ...
                            <tr class="border-bottom border-white border-opacity-5">
                                ...
                                <td class="text-center">
                                    @php
                                        ...
                                        // Intelligence: Auto-detected status based on missions
                                        $currentStatus = $driver->is_in_field ? 'on_delivery' : $driver->status;
                                        $s = $statusMap[$currentStatus] ?? ['class' => 'secondary', 'label' => $currentStatus];
                                    @endphp
                                    <div class="dropdown">
                                        <button
                                            class="badge bg-{{ $s['class'] }} bg-opacity-10 text-{{ $s['class'] }} border border-{{ $s['class'] }} border-opacity-25 rounded-pill px-3 py-2 dropdown-toggle shadow-none"
                                            type="button" data-bs-toggle="dropdown">
                                            {{ $s['label'] }}
                                        </button>
                                        <ul
                                            class="dropdown-menu dropdown-menu-end glass-card border-white border-opacity-10 shadow-lg">
                                            @foreach($statusMap as $val => $cfg)
                                                <li>
                                                    <form action="{{ route('hr.delivery.status', $driver) }}" method="POST">
                                                        ...
                                                        <button type="submit"
                                                            class="dropdown-item text-{{ $cfg['class'] }} small d-flex align-items-center gap-2">
                                ...
                                <td class="pe-4 text-end">
                                    <div class="d-flex justify-content-end gap-2">
                                        ...
                                        <form action="{{ route('hr.delivery.destroy', $driver) }}" method="POST"
                                            class="d-inline"
