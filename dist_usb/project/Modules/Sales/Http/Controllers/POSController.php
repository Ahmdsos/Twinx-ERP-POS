...
class POSController extends Controller
{
    ...
    {
        $shift->load(['user', 'invoices.customer', 'invoices.user', 'invoices.lines', 'expenses', 'returns']);
        ...
        $stats = [
            ...
            'total_sales' => (float) $shift->actual_gross_total,
            'net_revenue' => (float) $shift->actual_net_revenue,
            'total_cash' => (float) $shift->actual_cash_total,
            'total_card' => (float) $shift->invoices()
                ->where('status', '!=', \Modules\Sales\Enums\SalesInvoiceStatus::CANCELLED)
                ->get()
                ->sum(fn($i) => collect($i->details['payments'] ?? [])->where('method', 'card')->sum('amount')),
            'total_credit' => (float) $shift->invoices()->sum('balance_due'),
            'total_expenses' => (float) $shift->actual_expenses_total,
            'total_returns' => (float) $shift->actual_returns_total,
            'expected_cash' => (float) $shift->getCurrentBalance(),
    ...
    public function searchProducts(Request $request)
    {
        ...
        $products = Product::with([
            ...
            ->when($query, function ($q) use ($query, $request) {
                // Phase 15: Exact Match for Scanner
    ...
    public function searchCustomers(Request $request)
    {
        ...
        $customers = \Modules\Sales\Models\Customer::active()
            ...
            ->limit(10)
            ->get(['id', 'name', 'mobile', 'phone', 'code', 'credit_limit', 'shipping_address', 'billing_address', 'shipping_city', 'billing_city']);

        // Append computed 'balance' accessor (not a real DB column)
        $customers->each(fn($c) => $c->append('balance'));
    ...
    {
        // IDOR Mitigation: Ensure user has permission and customer is active
        if (!$request->user()->can('sales.create')) {
            abort(403, 'Unauthorized access to customer data.');
        }

        $customer = \Modules\Sales\Models\Customer::where('id', $id)
            ->active()
            ->firstOrFail();
    ...
    {
        $invoice->load(['customer', 'lines.product', 'creator', 'paymentAllocations.payment', 'deliveryOrder']);

        if ($invoice->is_delivery) {
            $settings = [
                'logo' => \App\Models\Setting::getValue('company_logo') ? asset('storage/' . \App\Models\Setting::getValue('company_logo')) : null,
                'company_name' => \App\Models\Setting::getValue('company_name', config('app.name')),
                'address' => \App\Models\Setting::getValue('company_address', ''),
                'phone' => \App\Models\Setting::getValue('company_phone', ''),
            ];
            return view('pos.receipt_delivery', compact('invoice', 'settings'));
        }
    ...
    public function holdSale(Request $request)
    {
        ...
        $heldSale = DB::transaction(function () use ($request) {
            return \Modules\Sales\Models\PosHeldSale::hold([
                'items' => $request->items,
        ...
        $heldCount = \Modules\Sales\Models\PosHeldSale::held()->count();
    ...
    public function resumeSale(Request $request)
    {
        ...
        $sale = \Modules\Sales\Models\PosHeldSale::held()
    ...
    public function openShift(Request $request)
    {
        ...
        $user = auth()->user();
        if (!$user->employee) {
            // SELF-HEALING for Admin User (Truth Rule)
            // If superadmin, we allow auto-linking or bypass to prevent system deadlocks
            if ($user->id === 1 || session('is_super_admin', false)) {
                $employee = \Modules\HR\Models\Employee::where('user_id', $user->id)->first();
                if (!$employee) {
                    // Try to find first employee with no user_id and hijack it, or create generic one
                    /** @var \Modules\HR\Models\Employee $employee */
                    $employee = \Modules\HR\Models\Employee::whereNull('user_id')->first();
                    if ($employee) {
                        $employee->update(['user_id' => $user->id]);
                    } else {
                        // Create a default employee profile for Admin to ensure SSOT
                        $employee = \Modules\HR\Models\Employee::create([
                            ...
                            'status' => \Modules\HR\Enums\EmployeeStatus::ACTIVE,
                            'date_of_joining' => now(),
                        ]);
                    }
                }
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'عفواً، حسابك غير مرتبط بملف موظف. يرجى مراجعة الموارد البشرية لربط حسابك بملف الموظف الخاص بك.'
                ], 403);
            }
        }
        ...
        if ($user->employee->status !== \Modules\HR\Enums\EmployeeStatus::ACTIVE) {
            return response()->json([
                'success' => false,
                'message' => 'عفواً، ملف الموظف الخاص بك غير نشط. يرجى التواصل مع إدارة الموارد البشرية.'
            ], 403);
        }
    ...
    public function closeShift(Request $request)
    {
        ...
        $closePin = \Modules\Core\Models\Setting::getValue('pos_close_pin', '1234');
    ...
    public function validateRefundPin(Request $request)
    {
        ...
        \Modules\Core\Models\SecurityAuditLog::logFailedPin(auth()->id(), 'refund_pin');
    ...
    public function salesReturn(Request $request)
    {
        ...
        if ($data['pin'] !== $storedPin) {
            \Modules\Core\Models\SecurityAuditLog::logFailedPin(auth()->id(), 'sales_return');
            return response()->json(['success' => false, 'message' => 'الرقم السري غير صحيح'], 403);
        }
        ...
        try {
            ...
            \Modules\Core\Models\SecurityAuditLog::logRefund(
                $data['invoice_id'],
    ...
    public function validatePriceOverridePin(Request $request)
    {
        ...
        // Get active shift
        $activeShift = \Modules\Sales\Models\PosShift::getActiveShift();

        if ($request->pin === $storedPin) {
            // Log successful override
            \Modules\Sales\Models\PriceOverrideLog::logOverride([
        ...
        // Log failed attempt
        \Modules\Core\Models\SecurityAuditLog::logFailedPin(auth()->id(), 'price_override');
    ...
    public function logCartDeletion(Request $request)
    {
        ...
        \Modules\Core\Models\SecurityAuditLog::logCartDelete(
            $request->product_id,
    ...
    public function xReport()
    {
        ...
        // SSOT: Use the dynamic calculation from the model
        $expectedCash = (float) $activeShift->getCurrentBalance();
        ...
        $stats = [
            ...
            // Sales Totals (SSOT - From Actual Records)
            'total_sales' => (float) $activeShift->actual_gross_total,
            'net_revenue' => (float) $activeShift->actual_net_revenue,
            'transaction_count' => (int) $activeShift->invoices()->count(),

            // Payment Breakdown (derived from actual payments)
            'total_cash' => (float) $activeShift->actual_cash_total,
            'total_card' => (float) $activeShift->invoices()
                ->where('status', '!=', \Modules\Sales\Enums\SalesInvoiceStatus::CANCELLED)
                ->get()
                ->sum(fn($i) => collect($i->details['payments'] ?? [])->where('method', 'card')->sum('amount')),
            'total_credit' => (float) $activeShift->invoices()->sum('balance_due'),
    ...
    public function saveExpense(Request $request)
    {
        ...
        try {
            // Use default Expense Category — MUST have account_id for journal posting
            $generalExpAccount = \Modules\Accounting\Models\Account::where('code', '5223')->first()
                ?? \Modules\Accounting\Models\Account::where('code', 'like', '522%')->where('is_header', false)->first()
                ?? \Modules\Accounting\Models\Account::where('code', 'like', '52%')->where('is_header', false)->first()
                ?? \Modules\Accounting\Models\Account::where('code', 'like', '5%')->where('is_header', false)->first();
            ...
            $category = \Modules\Finance\Models\ExpenseCategory::firstOrCreate(
                ...
                [
                    'code' => 'EXP-GEN',
                    'is_active' => true,
                    'account_id' => $generalExpAccount?->id,
                ]
            );

            // Ensure account_id is set even if category already exists without one
            if (!$category->account_id && $generalExpAccount) {
                $category->update(['account_id' => $generalExpAccount->id]);
            }
    ...
    public function openCashDrawer(Request $request)
    {
        ...
        \Modules\Core\Models\SecurityAuditLog::log(
            \Modules\Core\Models\SecurityAuditLog::EVENT_DRAWER_OPEN,
            'فتح درج النقدية يدوياً',
            ['reason' => $request->reason],
            \Modules\Core\Models\SecurityAuditLog::SEVERITY_INFO
        );
    ...
    /**
     * Daily summary report for POS
     */
    public function dailySummary()
    {
        $today = now()->format('Y-m-d');

        $invoices = SalesInvoice::whereDate('invoice_date', $today)
            ->where('source', 'pos')
            ->get();
        ...
        $stats = [
            ...
            'total_cash' => $invoices->sum(fn($i) => collect($i->details['payments'] ?? [])->where('method', 'cash')->sum('amount')),
            'total_card' => $invoices->sum(fn($i) => collect($i->details['payments'] ?? [])->where('method', 'card')->sum('amount')),
            'total_credit' => $invoices->sum('balance_due'),
        ];
