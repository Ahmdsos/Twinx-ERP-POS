...
@section('content')
                            ...
                            @forelse($stockValue as $item)
                                <tr class="product-row" data-name="{{ strtolower($item->name) }}"
                                    ...
                                    <td class="text-end pe-4 fw-bold text-success">
                                        {{ number_format($item->total_retail_value, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
    ...
    <script>
        function thermalPrintValuation() {
            const rows = [];
            @foreach($stockValue as $item)
                rows.push([
                    '{{ $item->name }}',
                    '{{ number_format($item->stock_quantity, 0) }}',
                    '{{ number_format($item->total_cost_value, 2) }}'
                ]);
            @endforeach
            ...
            printThermal({
                ...
                summaryCards: [
                    { label: 'قيمة التكلفة', value: '{{ number_format($totalCostValue, 2) }}' },
                    { label: 'قيمة البيع', value: '{{ number_format($totalRetailValue, 2) }}' },
                    { label: 'عدد الأصناف', value: '{{ $stockValue->count() }}' },
                ],
                sections: [
                    {
                        title: 'الأصناف',
                        headers: ['المنتج', 'كمية', 'قيمة التكلفة'],
                        rows: rows,
                        footer: { label: 'إجمالي التكلفة', value: '{{ number_format($totalCostValue, 2) }}' }
                    }
                ]
            });
        }
