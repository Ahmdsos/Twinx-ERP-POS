    ...
    public function pay(Request $request, Advance $advance)
    {
        ...
        try {
            DB::transaction(function () use ($request, $advance) {
                $treasuryAccountCode = Setting::getValue('acc_cash', '1101');
                $advancesAccountCode = Setting::getValue('acc_employee_advances', '1210');
                ...
                $treasuryAccount = Account::where('code', $treasuryAccountCode)->first();
                $advancesAccount = Account::where('code', $advancesAccountCode)->first();

                if (!$treasuryAccount || !$advancesAccount) {
                    throw new \Exception("حسابات الخزينة أو السلف غير موجودة في دليل الحسابات. يرجى مراجعة الإعدادات.");
                }
