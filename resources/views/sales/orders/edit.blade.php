...
@section('title', 'تعديل أمر بيع ' . $salesOrder->so_number)

@section('content')
<div class="container-fluid p-0" x-data="salesOrderBuilder({{ $salesOrder->lines->map(function($line) {
    return [
...
}) }}, {{ $salesOrder->discount_amount ?? 0 }}, '{{ $salesOrder->warehouse_id }}')">
    ...
    <form action="{{ route('sales-orders.update', $salesOrder->id) }}" method="POST" id="order-form" @keydown.enter.prevent>
        @csrf
        @method('PUT')
...
<script>
    function salesOrderBuilder(initialItems = [], initialDiscount = 0, initialWarehouse = '') {
        return {
