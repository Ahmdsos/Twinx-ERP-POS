    ...
    <div class="col-12">
        <form action="{{ route('sales-returns.store') }}" method="POST" @submit.prevent="submitForm">
            ...
            <div class="glass-card p-4 mb-4">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select name="customer_id" x-model="customerId" @change="loadInvoices" class="form-select form-select-lg bg-transparent text-white border-secondary" required>
                    ...
                    <div class="col-md-3">
                         ...
                         <select name="sales_invoice_id" x-model="invoiceId" @change="loadInvoiceItems" class="form-select bg-transparent text-white border-secondary" :disabled="!customerId">
                            <option value="">-- بدون فاتورة (مرتجع حر) --</option>
                            <template x-for="invoice in invoices" :key="invoice.id">
                                <option :value="invoice.id" x-text="'#' + invoice.invoice_number + ' (' + invoice.invoice_date + ')'"></option>
                            </template>
                         </select>
                    ...
                    <div class="col-md-3">
                        <label class="form-label text-gray-300">المخزن (الاستلام) <span class="text-danger">*</span></label>
                        <select name="warehouse_id" class="form-select bg-transparent text-white border-secondary" required>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="date" name="return_date" class="form-control bg-transparent text-white border-secondary" value="{{ date('Y-m-d') }}" required>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card p-4 mb-4">
                        ...
                        <tbody>
                            <template x-for="(item, index) in items" :key="index">
                                <tr class="border-bottom border-secondary border-opacity-10">
                                    <td>
                                        <select :name="'items['+index+'][product_id]'" x-model="item.product_id" class="form-select bg-transparent text-white border-secondary fs-6" required>
                                    ...
                                    <td>
                                        <select :name="'items['+index+'][condition]'" x-model="item.condition" class="form-select bg-transparent text-white border-secondary fs-6">
                                            <option value="resalable">صالح للبيع</option>
                                            <option value="damaged">تالف / معيب</option>
                                        </select>
                                    </td>
                                    <td>
                                        <input type="number" step="0.01" :name="'items['+index+'][quantity]'" x-model="item.quantity" class="form-control bg-transparent text-white border-secondary text-center" min="0.01" required>
                                    </td>
                                    <td>
                                        <input type="number" step="0.01" :name="'items['+index+'][unit_price]'" x-model="item.unit_price" class="form-control bg-transparent text-white border-secondary text-center" min="0" required>
                                    </td>
                                    <td class="text-end fw-bold text-info">
                                        <span x-text="(item.quantity * item.unit_price).toFixed(2)"></span>
                                    </td>
                                    <td class="text-end">
                                        <button type="button" @click="removeItem(index)" class="btn btn-sm btn-icon-glass text-danger hover-danger">
                        ...
                        <tfoot class="border-top border-secondary border-opacity-25">
                            <tr>
                                ...
                                <td class="text-end py-3 text-white fw-bold fs-5">
                                    <span x-text="grandTotal"></span> <small class="fs-6 fw-normal text-gray-500">ج.م</small>
                                </td>
                                <td></td>
                            </tr>
                        </tfoot>
                ...
                <div x-show="items.length === 0" class="text-center py-5 text-gray-500 border border-dashed border-secondary border-opacity-25 rounded-3 mt-3">
            ...
            <div class="glass-card p-4">
                ...
                <textarea name="notes" class="form-control bg-transparent text-white border-secondary" rows="3" placeholder="أدخل أي تفاصيل إضافية حول سبب الإرجاع..."></textarea>
            </div>
        </form>
    </div>
</div>
        ...
        return {
            ...
            async loadInvoiceItems() {
                ...
                try {
                    ...
                    alert('حدث خطأ أثناء تحميل أصناف الفاتورة');
                    this.addItem(); 
                } finally {
