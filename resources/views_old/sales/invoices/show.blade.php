                                ...
                                @foreach($salesInvoice->lines as $index => $line)
                                    <tr>
                                        ...
                                        <td>{{ number_format($line->quantity, 2) }}
                                            {{ $line->product?->unit?->abbreviation ?? '' }}
                                        </td>
                                ...
                                @if($salesInvoice->discount_amount > 0)
                                    <tr>
                                        ...
                                        <td class="text-danger">
                                            <strong>-{{ number_format($salesInvoice->discount_amount, 2) }}</strong>
                                        </td>
                                    </tr>
                                @endif
        ...
        <div class="col-lg-4">
            ...
            <div class="card mb-4">
                <div class="card-header bg-secondary text-white">
                    <h6 class="mb-0"><i class="bi bi-journal-text me-2"></i>الحالة المحاسبية</h6>
                </div>
                <div class="card-body">
                    @if($salesInvoice->journalEntry)
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span>رقم القيد:</span>
                            <a href="{{ route('journal-entries.show', $salesInvoice->journal_entry_id) }}"
                                class="fw-bold text-decoration-none">
                                {{ $salesInvoice->journalEntry->entry_number }}
                            </a>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span>الحالة:</span>
                            @if($salesInvoice->journalEntry->status == 'posted')
                                <span class="badge bg-success">مرحل (Posted)</span>
                            @else
                                <span class="badge bg-warning text-dark">مسودة (Draft)</span>
                            @endif
                        </div>
                    @else
                        <div class="text-center text-muted">
                            <i class="bi bi-journal-x fs-1 d-block mb-2"></i>
                            لا يوجد قيد يومية مرتبط
                            @if($salesInvoice->status->value !== 'cancelled')
                                <div class="mt-2">
                                    <small class="text-danger d-block">يجب مراجعة الإعدادات</small>
                                </div>
                            @endif
                        </div>
                    @endif
