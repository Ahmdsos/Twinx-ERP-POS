...
class CustomerController extends Controller
{
    ...
    public function statement(Request $request, Customer $customer)
    {
        ...
        $returns = $customer->salesReturns()
            ->whereBetween('return_date', [$fromDateTime, $toDateTime])
        ...
        // Merge and sort
        $transactions = $invoices->concat($payments)->concat($returns)->sortBy('date')->values();
        ...
        // 2. Payments before FROM date 
        $openingPayments = $customer->payments()
            ->where('payment_date', '<', $fromDateTime)
            ->sum('amount');

        // 3. Returns before FROM date
        $openingReturns = $customer->salesReturns()
            ->where('return_date', '<', $fromDateTime)
            ->where('status', \Modules\Sales\Enums\SalesReturnStatus::APPROVED)
            ->sum('total_amount');

        $openingBalance = $openingInvoices - $openingPayments - $openingReturns;

        // Calculate totals for period
        $totalInvoices = $invoices->sum('debit');
        $totalPaymentsExceptReturns = $payments->sum('credit');
        $totalReturns = $returns->sum('credit');
        $totalPayments = $totalPaymentsExceptReturns + $totalReturns;
    ...
    public function creditHistory(Customer $customer)
    {
        ...
        $stats = (object) [
            ...
            'total_amount' => $invoices->sum('total') - $totalReturns,
