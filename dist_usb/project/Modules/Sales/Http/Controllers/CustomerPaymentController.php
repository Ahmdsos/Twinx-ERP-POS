...
class CustomerPaymentController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        // CRITICAL: Only show Cash (1101) and Bank (11xx) accounts.
        // Exclude AR (1201), Employee Advances (1210), Inventory (1301), etc.
        // to prevent self-canceling journal entries (DR AR / CR AR).
        $paymentAccounts = Account::whereIn('type', ['asset'])
            ->where('code', 'like', '11%') // Only Cash & Bank accounts
    ...
    public function store(Request $request)
    {
        ...
        // 1. Handle explicit allocations from form (if any)
        if (!empty($validated['allocations'])) {
            foreach ($validated['allocations'] as $alloc) {
                if (!empty($alloc['amount']) && $alloc['amount'] > 0) {
                    // Correct Format: Array of arrays for SalesService
                    $invoiceAllocations[] = [
        ...
        // 2. Handle Auto-Allocation from "Pay Invoice" link (target_invoice_id)
        elseif ($request->filled('target_invoice_id')) {
            $invoiceAllocations[] = [
                'invoice_id' => $request->target_invoice_id,
                'amount' => $validated['amount'] // Allocate full amount to this invoice
            ];
        }
        ...
        $payment = $this->salesService->receivePayment(
            ...
            paymentAccount: $paymentAccount, // corrected argument name order
    ...
    public function edit(CustomerPayment $customerPayment)
    {
        return redirect()->route('customer-payments.show', $customerPayment)
            ->with('info', 'تعديل الدفعة غير متاح - يرجى إنشاء دفعة جديدة');
    }
    ...
    {
        return redirect()->route('customer-payments.show', $customerPayment)
            ->with('info', 'تعديل الدفعة غير متاح');
    }
    ...
    {
        return back()->with('error', 'لا يمكن حذف دفعة مسجلة - يرجى التواصل مع الإدارة');
    }
}
