            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <div class="d-flex align-items-center gap-3 mb-1">
                        <h2 class="fw-bold text-heading mb-0">{{ $salesOrder->so_number }}</h2>
                        <span
                            class="badge {{ $salesOrder->status->badgeClass() }} border border-secondary border-opacity-10/20 fs-6 px-3 py-2 rounded-pill shadow-sm">
                            {{ $salesOrder->status->label() }}
                        </span>
                    </div>
                    <div class="d-flex gap-3 text-gray-400 small">
                        <span><i class="bi bi-calendar me-1"></i> تاريخ الطلب: <span
                                class="text-body">{{ $salesOrder->order_date->format('Y-m-d') }}</span></span>
                        <span class="text-info"><i class="bi bi-clock-history me-1"></i> التسليم المتوقع: <span
                                class="text-body fw-bold">{{ $salesOrder->expected_date ? $salesOrder->expected_date->format('Y-m-d') : 'غير محدد' }}</span></span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-8">
                ...
                <div class="glass-panel p-0 rounded-4 overflow-hidden border border-secondary border-opacity-10/10 shadow-lg mb-4">
                    <div class="bg-surface/5 p-4 border-bottom border-secondary border-opacity-10/10">
                        <h5 class="fw-bold text-heading mb-0"><i class="bi bi-list-check me-2 text-info"></i> تفاصيل الطلب
                        </h5>
                    </div>
                            ...
                            <tbody>
                                @foreach($salesOrder->lines as $line)
                                    <tr class="hover:bg-surface/5 border-bottom border-secondary border-opacity-10/5">
                                        ...
                                        <td class="text-center py-3">
                                            <span
                                                class="badge {{ $line->delivered_quantity >= $line->quantity ? 'bg-success/20 text-success' : 'bg-warning/20 text-warning' }} border border-secondary border-opacity-10/10 rounded-pill px-3">
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="glass-panel p-4 rounded-4 h-100 border border-secondary border-opacity-10/10">
                            ...
                            <p class="text-body mb-0 small opacity-75">{{ $salesOrder->notes ?: 'لا توجد ملاحظات' }}</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="glass-panel p-4 rounded-4 h-100 border border-secondary border-opacity-10/10">
                            <h6 class="text-gray-400 fw-bold mb-3 small text-uppercase"><i class="bi bi-truck me-2"></i>
                                معلومات الشحن</h6>
                            <p class="text-body mb-2 small"><strong>العنوان:</strong>
                                {{ $salesOrder->shipping_address ?: 'نفس عنوان العميل' }}</p>
                            <p class="text-body mb-0 small"><strong>الطريقة:</strong>
            ...
            <div class="col-lg-4">
                ...
                <div
                    ...
                    <div class="d-flex align-items-center gap-3 mb-4">
                        ...
                        <div>
                            <h5 class="fw-bold text-heading mb-0">{{ $salesOrder->customer->name }}</h5>
                    ...
                    <div class="vstack gap-3 text-gray-300 small">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <span
                                class="badge bg-surface/5 border border-secondary border-opacity-10/10">{{ $salesOrder->customer->type_label }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="glass-panel p-4 rounded-4 border border-secondary border-opacity-10/10 shadow-lg">
                    ...
                    <div class="vstack gap-3 small">
                        <div class="d-flex justify-content-between text-gray-400">
                            <span>المخزن:</span>
                            <span class="text-body">{{ $salesOrder->warehouse->name ?? '-' }}</span>
                        </div>
                        <div class="d-flex justify-content-between text-gray-400">
                            <span>المرجع (عرض سعر):</span>
                            <span
                                class="text-body">{{ $salesOrder->quotation_id ? 'QT-' . $salesOrder->quotation_id : '-' }}</span>
                        </div>
                        <div class="d-flex justify-content-between text-gray-400">
                            <span>تاريخ الإنشاء:</span>
                            <span class="text-body">{{ $salesOrder->created_at->format('Y-m-d H:i') }}</span>
                        </div>
                    </div>
                </div>
            </div>
