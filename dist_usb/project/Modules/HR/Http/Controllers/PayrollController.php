...
class PayrollController extends Controller
{
    ...
    /**
     * Show form for creating a new payroll (redirects to generate).
     */
    public function create()
    {
        return redirect()->route('hr.payroll.index')
            ->with('info', 'لإنشاء مسيرة رواتب جديدة، استخدم نموذج التوليد في صفحة المسيرات');
    }

    /**
     * Store a new payroll (alias for generate).
     */
    ...
    /**
     * Show form for editing payroll.
     */
    public function edit(Payroll $payroll)
    {
        if ($payroll->status !== 'draft') {
            return redirect()->route('hr.payroll.show', $payroll)
                ->with('error', 'لا يمكن تعديل مسيرة رواتب معتمدة أو مرحلة');
        }
        return redirect()->route('hr.payroll.show', $payroll);
    }
    ...
    {
        if ($payroll->status !== 'draft') {
            return back()->with('error', 'لا يمكن تعديل مسيرة رواتب معتمدة أو مرحلة');
        }
        return redirect()->route('hr.payroll.show', $payroll)
            ->with('info', 'لتعديل البنود، استخدم أزرار التعديل الفردية في صفحة التفاصيل');
    }
    ...
    {
        if ($payroll->status !== 'draft') {
            return back()->with('error', 'لا يمكن حذف مسيرة رواتب معتمدة أو مرحلة');
        }
        $payroll->items()->delete();
        $payroll->delete();
        return redirect()->route('hr.payroll.index')->with('success', 'تم حذف مسيرة الرواتب بنجاح');
    }
}
