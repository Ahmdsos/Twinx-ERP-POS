...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <div class="text-muted small">حجم التعاملات والمديونيات</div>
            </div>
            ...
            <div class="d-flex gap-2 align-items-center">
                <form action="{{ route('reports.purchases.by-supplier') }}" method="GET"
                    ...
                    <input type="date" name="end_date" value="{{ $endDate }}"
                        class="form-control form-control-sm bg-transparent text-body border-0">
                    <button type="submit" class="btn btn-sm btn-warning text-body px-3 fw-bold">{{ __('Filter') }}</button>
                </form>
                ...
                <button class="btn btn-sm btn-outline-warning" onclick="thermalPrintBySupplier()" title="طباعة حرارية 80mm">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-3">
                <div class="card glass-card border-0 text-center py-3">
                    <div class="text-muted small text-uppercase">إجمالي المشتريات</div>
                    <h3 class="fw-bold text-heading mb-0 text-shadow">{{ number_format($data->sum('total_purchases'), 2) }}
                    </h3>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card glass-card border-0 text-center py-3">
                    <div class="text-muted small text-uppercase">مستحقات الموردين</div>
                    ...
                    <table class="table table-dark table-hover table-transparent align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($data as $supplier)
                                <tr>
                                    <td class="ps-4 fw-bold text-body">{{ $supplier->supplier_name }}</td>
                                    ...
                                    <td class="text-end fw-bold text-body">{{ number_format($supplier->total_purchases, 2) }}
                                    </td>
    ...
    <script>
        function thermalPrintBySupplier() {
            const rows = [];
            ...
            printThermal({
                title: 'تحليل مشتريات الموردين',
                subtitle: 'من {{ $startDate }} إلى {{ $endDate }}',
                summaryCards: [
                    { label: 'إجمالي المشتريات', value: '{{ number_format($data->sum("total_purchases"), 2) }}' },
                    { label: 'مستحقات الموردين', value: '{{ number_format($data->sum("total_due"), 2) }}' },
                ],
                sections: [
                    {
                        title: 'الموردين',
                        headers: ['المورد', 'فواتير', 'المشتريات'],
                        rows: rows,
                        footer: { label: 'الإجمالي', value: '{{ number_format($data->sum("total_purchases"), 2) }}' }
                    }
                ]
            });
        }
