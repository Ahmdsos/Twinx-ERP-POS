...
class ExpenseController extends Controller
{
    ...
    public function edit(Expense $expense)
    {
        $categories = ExpenseCategory::where('is_active', true)->get();
        $paymentAccounts = Account::where('is_active', true)->get();
        return view('finance.expenses.edit', compact('expense', 'categories', 'paymentAccounts'));
    }
    ...
    public function update(Request $request, Expense $expense)
    {
        ...
        return DB::transaction(function () use ($request, $expense, $validated, $taxAmount, $totalAmount) {
            // 1. If expense has a journal entry, reverse it
            if ($expense->journal_entry_id) {
                $oldEntry = \Modules\Accounting\Models\JournalEntry::find($expense->journal_entry_id);
                if ($oldEntry) {
                    $journalService = app(JournalService::class);
                    $journalService->reverse($oldEntry, 'تعديل المصروف: ' . $expense->reference_number);
                }
            ...
            // 4. Create and post new journal entry
            $expenseService = app(\Modules\Finance\Services\ExpenseService::class);
            $expenseService->createJournalEntry($expense->fresh());
    ...
    {
        if ($expense->journal_entry_id) {
            return back()->with('error', 'لا يمكن حذف مصروف مرتبط بقيد محاسبي');
        }
        $expense->delete();
