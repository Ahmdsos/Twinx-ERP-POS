...
class PosShift extends Model
{
    ...
    public function getActualCashTotalAttribute(): float
    {
    ...
    public function getActualExpensesTotalAttribute(): float
    {
        return (float) $this->expenses()->sum('amount');
    }

    public function getActualReturnsTotalAttribute(): float
    {
        return (float) $this->returns()->sum('total_amount');
    }

    public function getActualGrossTotalAttribute(): float
    {
        $sales = (float) $this->invoices()
            ->where('status', '!=', \Modules\Sales\Enums\SalesInvoiceStatus::CANCELLED)
            ->sum('total');

        return $sales - $this->actual_returns_total;
    }

    public function getActualNetRevenueAttribute(): float
    {
        ...
        // Subtract subtotal of returns (Revenue portion)
        $returnRevenue = (float) $this->returns()->sum('subtotal');

        return $revenue - $returnRevenue;
    }

    public function getCurrentBalance(): float
    {
        // SSOT: Calculate from actual records instead of redundant counters
        return ($this->opening_cash + $this->actual_cash_total) - $this->actual_expenses_total - $this->actual_returns_total;
    }
