...
@section('content')
    <div class="container-fluid p-0">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 d-print-none">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h2 class="fw-bold text-white mb-0">سند صرف نقدية / بنك</h2>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="glass-panel p-0 overflow-hidden border-top-4 border-purple-500 position-relative">
                    <!-- Receipt Header -->
                    <div class="bg-white-5 p-4 border-bottom border-white-5 text-center">
                        <div class="mb-3">
                            <i class="bi bi-check-circle-fill text-green-400 fs-1"></i>
                        </div>
                        <h3 class="text-white fw-bold mb-1">تم الدفع بنجاح</h3>
                        <p class="text-gray-400 mb-0">{{ $supplierPayment->payment_date->format('d F Y, h:i A') }}</p>
                    </div>

                    <!-- Amount -->
                    <div class="p-5 text-center border-bottom border-white-5">
                        <span class="text-purple-300 small text-uppercase tracking-wide fw-bold">المبلغ المدفوع</span>
                        <h1 class="display-4 fw-bold text-white mb-0 mt-2">{{ number_format($supplierPayment->amount, 2) }} <span class="fs-4 text-gray-500">EGP</span></h1>
                        <div class="mt-3">
                            @if($supplierPayment->payment_method == 'cash')
                                <span class="badge bg-green-500 bg-opacity-10 text-green-400 border border-green-500 border-opacity-20 px-3 py-2 rounded-pill">
                                    <i class="bi bi-cash me-2"></i>نقدي
                                </span>
                            ...
                            @else
                                <span class="badge bg-orange-500 bg-opacity-10 text-orange-400 border border-orange-500 border-opacity-20 px-3 py-2 rounded-pill">
                                    <i class="bi bi-card-checklist me-2"></i>شيك
                                </span>
                            @endif
                        </div>
                    </div>
                        ...
                        <div class="row g-4">
                            <div class="col-6">
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    <div class="avatar-xs bg-purple-500 rounded-circle text-white d-flex align-items-center justify-content-center fw-bold" style="width: 24px; height: 24px; font-size: 10px;">
                                        {{ strtoupper(substr($supplierPayment->supplier->name, 0, 1)) }}
                                    </div>
                                    <a href="{{ route('suppliers.show', $supplierPayment->supplier_id) }}" class="text-white fw-bold text-decoration-none">
                            ...
                            <div class="col-6">
                                ...
                                <p class="text-white fw-bold mb-0">{{ $supplierPayment->paymentAccount->name ?? '-' }}</p>
                            </div>
                            <div class="col-6">
                                <label class="text-gray-500 x-small fw-bold text-uppercase mb-1">رقم المرجع</label>
                                <p class="text-white font-monospace mb-0">{{ $supplierPayment->reference ?? '-' }}</p>
                            </div>
                            <div class="col-6">
                                <label class="text-gray-500 x-small fw-bold text-uppercase mb-1">المستخدم</label>
                                <p class="text-white mb-0">{{ $supplierPayment->creator->name ?? 'System' }}</p>
                            </div>
                            @if($supplierPayment->notes)
                            <div class="col-12 border-top border-white-5 pt-3 mt-2">
                                <label class="text-gray-500 x-small fw-bold text-uppercase mb-1">ملاحظات</label>
                    ...
                    <div class="p-4 border-top border-white-5">
                        <h6 class="text-white fw-bold mb-3 small opacity-75">سداد مقابل الفواتير:</h6>
                        <div class="table-responsive">
                            <table class="table table-sm table-dark-custom mb-0">
                                <thead>
                                    <tr>
                                        <th class="text-gray-500 x-small">رقم الفاتورة</th>
                                ...
                                <tbody>
                                    @foreach($supplierPayment->allocations as $allocation)
                                        <tr>
                                            ...
                                            <td class="text-end fw-bold text-white">{{ number_format($allocation->amount, 2) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    <style>
        .btn-action-purple {
            ...
            border: none; color: white; padding: 8px 20px; border-radius: 8px; transition: all 0.3s;
        }
