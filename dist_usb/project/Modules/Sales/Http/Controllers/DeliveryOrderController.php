...
class DeliveryOrderController extends Controller
{
    ...
    /**
     * Show form for editing delivery order
     */
    public function edit(DeliveryOrder $delivery)
    {
        return redirect()->route('deliveries.show', $delivery)
            ->with('info', 'تعديل أمر التسليم غير متاح - يرجى إلغاء الأمر وإنشاء واحد جديد');
    }
    ...
    {
        return redirect()->route('deliveries.show', $delivery)
            ->with('info', 'تعديل أمر التسليم غير متاح');
    }
    ...
    {
        if ($delivery->status !== \Modules\Sales\Enums\DeliveryStatus::DRAFT) {
            return back()->with('error', 'لا يمكن حذف أمر تسليم تم شحنه أو تسليمه');
        }
        $delivery->lines()->delete();
        $delivery->delete();
        return redirect()->route('deliveries.index')->with('success', 'تم حذف أمر التسليم بنجاح');
    }
}
