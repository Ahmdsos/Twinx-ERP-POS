...
@section('content')
    <div class="container pb-5">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <div>
                <h4 class="mb-1 fw-bold text-heading"><i class="bi bi-scale me-2 text-info"></i> تحليل النسخة الاحتياطية</h4>
                <p class="text-body-50 mb-0">مقارنة تفصيلية قبل اتخاذ قرار الاستعادة</p>
            </div>
            <div>
        ...
        <div class="row g-4 mb-5">
            ...
            <div class="col-md-6">
                <div class="glass-card h-100 p-0 overflow-hidden border border-success border-opacity-50 shadow-lg">
                    <div
                        class="bg-success bg-opacity-25 p-4 border-bottom border-success border-opacity-25 d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            <i class="bi bi-hdd-rack-fill fs-3 text-success me-3"></i>
                            <div>
                                <h5 class="fw-bold text-heading mb-0">النظام الحالي (Live)</h5>
                    ...
                    <div class="p-5">
                        <div class="row g-5">
                            <div class="col-6">
                                ...
                                <div class="fs-2 fw-bold text-body font-monospace">
                                    {{ \Modules\Core\Http\Controllers\BackupController::formatBytes($liveStats['size']) }}
                                </div>
                            </div>
                            <div class="col-6">
            ...
            <!-- Backup Side -->
            <div class="col-md-6">
                <div class="glass-card h-100 p-0 overflow-hidden border border-warning border-opacity-50 shadow-lg"
                    style="background: rgba(30, 30, 30, 0.95);">
                    <div
                        class="bg-warning bg-opacity-10 p-4 border-bottom border-warning border-opacity-10 d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center">
                            <i class="bi bi-filetype-sql fs-3 text-warning me-3"></i>
                            <div>
                                <h5 class="fw-bold text-heading mb-0">نسخة الاستعادة</h5>
                                <small class="text-warning font-monospace dir-ltr">{{ $backupStats['name'] }}</small>
                            </div>
                        </div>
                        <span class="badge bg-warning text-body px-3 py-2">Backup</span>
                    </div>
                    ...
                    <div class="p-5">
                        <div class="row g-5">
                            <div class="col-6">
                                ...
                                <div class="fs-2 fw-bold text-body font-monospace">
                                    {{ \Modules\Core\Http\Controllers\BackupController::formatBytes($backupStats['size']) }}
                                </div>

                                <!-- Size Comparison Indicator -->
                                @if($backupStats['size'] > $liveStats['size'])
                                    <div class="badge bg-success bg-opacity-25 text-success mt-2">
                                        <i class="bi bi-arrow-up"></i> أكبر بـ
                                        {{ \Modules\Core\Http\Controllers\BackupController::formatBytes($backupStats['size'] - $liveStats['size']) }}
                                    </div>
                                @elseif($backupStats['size'] < $liveStats['size'])
                                    <div class="badge bg-danger bg-opacity-25 text-danger mt-2">
                                        <i class="bi bi-arrow-down"></i> أصغر بـ
                                        {{ \Modules\Core\Http\Controllers\BackupController::formatBytes($liveStats['size'] - $backupStats['size']) }}
                                    </div>
                                @else
                            ...
                            <div class="col-6">
                                ...
                                @if($backupStats['tables_count'] == $liveStats['tables_count'])
                                    <div class="text-success small fw-bold mt-2"><i class="bi bi-check-circle-fill"></i> هيكل
                                        مطابق</div>
                                @else
                                    <div class="text-warning small fw-bold mt-2"><i class="bi bi-exclamation-triangle-fill"></i>
                                        اختلاف هيكلي</div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="text-muted">تاريخ النسخة:</span>
                            </div>
                            <div class="text-end">
                                <div class="text-body fw-bold font-monospace mb-0">
                                    {{ date('Y-m-d H:i', $backupStats['date']) }}
                                </div>
                                <small
                                    class="text-secondary">{{ \Carbon\Carbon::createFromTimestamp($backupStats['date'])->diffForHumans() }}</small>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="text-center">
            <form action="{{ route('settings.backup.restore', $backupStats['name']) }}" method="POST">
                ...
                <div class="card bg-danger bg-opacity-10 border-danger border-opacity-25 d-inline-block p-4 rounded-4">
                    ...
                    <p class="text-body-50 mb-4" style="max-width: 400px; margin: 0 auto;">
                    ...
                    <button type="submit" class="btn btn-danger px-5 py-3 rounded-pill fw-bold hover-scale shadow-lg"
                        data-confirm="تأكيد نهائي: هل أنت متأكد من استبدال قاعدة البيانات؟">
