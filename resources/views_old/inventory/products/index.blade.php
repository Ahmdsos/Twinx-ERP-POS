                    ...
                    <tbody>
                        @forelse($products as $product)
                            <tr>
                                ...
                                <td>
                                    @php
                                        // Use the loaded accessor or relation to avoid N+1
                                        // Assuming stock_qty is appended or available
                                        $qty = $product->stock_qty ?? 0;
                                        $isLow = $qty <= 10;
                                    @endphp
                                    <span class="badge {{ $qty > 0 ? ($isLow ? 'bg-warning' : 'bg-success') : 'bg-danger' }}">
                                        {{ number_format($qty) }}
                                        @if($isLow && $qty > 0) <i class="bi bi-exclamation-triangle ms-1"
                                        title="مخزون منخفض"></i> @endif
                                    </span>
                                </td>
