...
class FinancialReportService
{
    ...
    protected function getAccountPrefixTotal(AccountType $type, array $prefixes, Carbon $fromDate, Carbon $toDate): float
    {
        ...
        foreach ($accountIds as $accountId) {
            ...
            // Apply sign based on account type normal balance
            $total += $this->normalizeBalance($balance, $type);
        }
    ...
    protected function getAccountTypeBalance(AccountType $type, Carbon $asOfDate): float
    {
        ...
        $total = 0;
        foreach ($accountIds as $accountId) {
            $balance = $this->getAccountBalance($accountId, null, $asOfDate);
            // Apply sign based on account type normal balance
            $total += $this->normalizeBalance($balance, $type);
        }
    ...
    protected function getAccountTypeBreakdown(AccountType $type, Carbon $fromDate, Carbon $toDate): array
    {
        ...
        $breakdown = [];
        foreach ($accounts as $account) {
            $rawBalance = $this->getAccountBalance($account->id, $fromDate, $toDate);
            // Apply same sign logic as getAccountTypeTotal
            if ($type === AccountType::REVENUE) {
                $balance = -$rawBalance; // credits - debits
            } else {
                $balance = $rawBalance; // debits - credits
            }

            // Show account if it has any activity (positive or negative)
    ...
    protected function getAccountTypeBreakdownBalance(AccountType $type, Carbon $asOfDate): array
    {
        ...
        $breakdown = [];
        foreach ($accounts as $account) {
            $rawBalance = $this->getAccountBalance($account->id, null, $asOfDate);
            $balance = $this->normalizeBalance($rawBalance, $type);
     ...
     * Revenue and Expenses are already sign-normalized by getAccountTypeBalance()
     */
    ...
    /**
     * Normalize a raw balance (debits - credits) based on account type.
     * 
     * Asset/Expense: normal balance is DEBIT → use as-is (debits - credits)
     * Liability/Equity/Revenue: normal balance is CREDIT → negate (credits - debits)
     * 
     * This ensures contra-accounts (e.g., Sales Returns, Accumulated Depreciation)
     * naturally reduce their parent category totals instead of inflating them.
     */
    protected function normalizeBalance(float $rawBalance, AccountType $type): float
    {
        return match ($type) {
            AccountType::ASSET, AccountType::EXPENSE => $rawBalance,
