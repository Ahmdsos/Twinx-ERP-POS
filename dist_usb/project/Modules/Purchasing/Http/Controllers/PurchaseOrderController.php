...
class PurchaseOrderController extends Controller
{
    ...
    {
        if ($purchaseOrder->status !== \Modules\Purchasing\Enums\PurchaseOrderStatus::DRAFT) {
            return back()->with('error', 'لا يمكن حذف أمر شراء غير مسودة');
        }
        $purchaseOrder->lines()->delete();
        $purchaseOrder->delete();
        return redirect()->route('purchase-orders.index')->with('success', 'تم حذف أمر الشراء بنجاح');
    }
}
