    ...
    <div class="container-fluid p-0">
        ...
        <div class="row g-4 mb-5">
            ...
            <div class="col-md-4">
                <div class="glass-card stat-card h-100 position-relative border-0 shadow-lg overflow-hidden">
                    <div class="position-absolute top-0 end-0 p-4 opacity-10">
                        <i class="bi bi-cash-stack display-1 text-white"></i>
                    </div>
                    <div class="p-4 position-relative z-1">
                        ...
                        <h1 class="text-white fw-bold mb-1 display-5">{{ number_format($totalToday, 2) }}</h1>
            ...
            <div class="col-md-4">
                <div class="glass-card stat-card h-100 position-relative border-0 shadow-lg overflow-hidden">
                    <div class="position-absolute top-0 end-0 p-4 opacity-10">
                        <i class="bi bi-bank display-1 text-white"></i>
                    </div>
                    <div class="p-4 position-relative z-1">
                        <div
                            class="badge bg-white/10 text-white mb-3 backdrop-blur-md border border-white/10 px-3 py-1 rounded-pill">
                        ...
                        <h1 class="text-white fw-bold mb-1 display-5">{{ number_format($totalMonth, 2) }}</h1>
            ...
            <div class="col-md-4">
                <div
                    class="glass-card h-100 p-4 d-flex flex-column justify-content-center align-items-center text-center border-dashed border-white/10">
                    <div class="icon-circle bg-white/5 mb-3 text-warning">
        ...
        <div class="glass-panel p-4 mb-4 rounded-4 border border-white/10 shadow-lg position-relative overflow-hidden">
            ...
            <form action="{{ route('customer-payments.index') }}" method="GET" class="position-relative z-1">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        <label class="text-gray-300 small fw-bold mb-2 ps-1">طريقة الدفع</label>
                        <select name="payment_method" class="form-select glass-select text-white shadow-none">
                            ...
                            <option value="bank_transfer" class="bg-gray-900" {{ request('payment_method') == 'bank_transfer' ? 'selected' : '' }}>تحويل بنكي</option>
                            <option value="check" class="bg-gray-900" {{ request('payment_method') == 'check' ? 'selected' : '' }}>شيك</option>
                        </select>
                    </div>
                ...
                <table class="table table-hover align-middle mb-0" style="border-collapse: separate; border-spacing: 0;">
                    <thead class="bg-white/5">
                        <tr>
                            ...
                            <th class="py-3 text-gray-400 fw-normal">طريقة الدفع</th>
                            <th class="py-3 text-gray-400 fw-normal">المرجع</th>
                            <th class="py-3 text-gray-400 fw-normal text-end">المبلغ</th>
                            ...
                            <tr class="hover-bg-white-5">
                                <td class="ps-4">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="font-monospace fw-bold text-white">{{ $payment->payment_number }}</span>
                                    </div>
                                </td>
                                <td>
                                    <span class="fw-bold text-white">{{ $payment->customer?->name ?? 'عميل غير معروف' }}</span>
                                </td>
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="text-white small">{{ $payment->payment_date->format('Y-m-d') }}</span>
                                ...
                                <td class="text-end">
                                    <h5 class="mb-0 fw-bold text-white text-glow">{{ number_format($payment->amount, 2) }}</h5>
                                </td>
                                <td class="pe-4 text-end">
                                    <div class="btn-group">
                                        <a href="{{ route('customer-payments.show', $payment->id) }}"
                                            class="btn btn-sm btn-icon-glass text-info" title="عرض التفاصيل">
                                            <i class="bi bi-eye-fill"></i>
                                        </a>
                                        <a href="{{ route('customer-payments.print', $payment->id) }}" target="_blank"
                                            class="btn btn-sm btn-icon-glass text-warning" title="طباعة الإيصال">
