...
@section('content')
    ...
    <div class="pos-workspace d-flex vh-100 overflow-hidden bg-slate-900" x-data="posStore()" x-init="initPOS()"
        ...
        <aside class="pos-cart-panel bg-slate-950 border-end border-slate-800 d-flex flex-column shadow-2xl z-30"
            ...
            <header class="p-3 bg-slate-900 border-bottom border-slate-800 shadow-sm">
                <!-- CUSTOMER SEARCH -->
                            ...
                            <div class="d-flex justify-content-between align-items-center h-100">
                                <div class="flex-grow-1">
                                    <span class="text-rose-400 d-block fs-[10px] text-uppercase mb-1"><i
                                            class="bi bi-tag-fill me-1"></i>خصم (EGP)</span>
                ...
                <div class="row g-2">
                    <div class="col-4">
                        <button
                            ...
                            <span class="text-[10px] fw-bold">المعلقة</span>
                        </button>
                    </div>
                    <div class="col-4">
                        <button
                            ...
                            <span class="text-[10px] fw-bold">تعليق</span>
                        </button>
                    </div>
                    <div class="col-4">
                        <button
                            ...
                            <span class="text-[10px] fw-bold">إلغاء</span>
                        </button>
                    </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center px-3 relative z-10">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div class="text-start lh-1">
                                        <span class="d-block fs-5 fw-black">دفع وسداد</span>
        ...
        <main
            ...
            <div
                class="px-5 py-3.5 bg-slate-900 border-bottom border-slate-800 d-flex gap-4 sticky-top z-20 align-items-center justify-content-between">
                ...
                <div class="d-flex gap-3 align-items-center">
                    ...
                    <button
                        ...
                        @click="openRecentTransactions()" title="سجل العمليات">
            ...
            <script>    function posStore() {
                    return {
                        ...
                        async fetchQuotationPrices() {
                            ...
                            try {
                                const url = `/pos/customers/${this.selectedCustomer.id}/quotation-prices`;
                        ...
                        addPayment() {
                            ...
                            if (method === 'credit' && !this.selectedCustomer) {
                                Swal.fire({ icon: 'warning', title: 'مطلوب عميل', text: 'يجب اختيار عميل للدفع الآجل', background: '#1e293b', color: '#fff' });
                                return;
                            }
                        ...
                        async fetchProducts() {
                            ...
                            try {
                                ...
                                Swal.fire({ icon: 'error', title: 'خطأ في التحميل', text: 'فشل تحميل المنتجات: ' + msg, background: '#1e293b', color: '#fff' });
                            } finally {
                        ...
                        addToCart(product) {
                            ...
                            if (product.stock <= 0) {
                                Swal.fire({ icon: 'warning', title: 'نفد المخزون', text: '⚠️ هذا المنتج نفد من المخزون', background: '#1e293b', color: '#fff' });
                                return;
                            }
                            if (currentQty + 1 > product.stock) {
                                Swal.fire({ icon: 'warning', title: 'كمية محدودة', text: `⚠️ الكمية المتاحة من (${product.name}) هي ${product.stock} فقط`, background: '#1e293b', color: '#fff' });
                                return;
                            }
                        ...
                        async holdSale() {
                            ...
                            Swal.fire({
                                ...
                                text: 'يمكنك استعادة الطلب لاحقاً من قائمة الفواتير المعلقة.',
                                background: '#1e293b',
                                color: '#fff',
                        ...
                        async clearCart() {
                            const result = await window.confirmAction({
                                title: 'إلغاء الطلب',
                                text: 'سيتم مسح جميع الأصناف من السلة، هل أنت متأكد؟',
                            ...
                            try {
                                ...
                                if (res.data.success) {
                                    ...
                                    Swal.fire({
                                        ...
                                        text: 'فاتورة رقم: ' + res.data.invoice.number,
                                        showCancelButton: true,
                                        confirmButtonColor: '#10b981',
                                        cancelButtonColor: '#6b7280',
                                        confirmButtonText: '<i class="bi bi-printer me-2"></i> طباعة الفاتورة',
                        ...
                        async quickCreateCustomer() {
                            ...
                            } catch (e) { Swal.fire({ icon: 'error', title: 'خطأ', text: 'فشل إضافة العميل', background: '#1e293b', color: '#fff' }); }
                        },
                        ...
                        async handleBarcodeScan(code) {
                            ...
                            try {
                                ...
                                if (products.length === 1) {
                                    ...
                                    // Clear search to be ready for next scan
                                    this.productSearch = '';
                                    this.products = [];
